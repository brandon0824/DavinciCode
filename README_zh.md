...
* **🃏 私密开局整理、黑白双任意百搭牌 (`-` 牌) 与自由插牌规则**：
  * **26 张完整牌堆**：包含黑色 0-11（12 张）+ 白色 0-11（12 张），以及 **1 张黑色任意百搭牌 `-`** 和 **1 张白色任意百搭牌 `-`**。
  * **开局私密整理**：对局开始后，每位玩家先私密整理初始手牌中的任意百搭牌并确认。此阶段其他玩家不会收到其手牌数据，因此无法从确认时机推断谁摸到了百搭牌。
  * **自由插入任意位置**：遵循《达芬奇密码》官方经典规则，当玩家摸到或手牌持有任意百搭牌 `-` 时，可**自由选择插入手牌的任意缝隙位置**（最左侧、卡牌之间或最右侧）混淆对手心理博弈；手牌区提供“`⇄ 位置`”微调功能。

* **🏆 本局击杀榜与猜测结果提示**：
  * 系统会统计本局每位玩家猜中、猜错次数，并在房间内“击杀榜”和结算复盘中展示。
  * 新产生的猜测结果会向所有在线玩家显示短暂提示；房间日志仍是完整的权威记录。
...
* **⚠️ 房间玩家 5 分钟断线/关窗口自动踢出 & 动态日志**：
  * 房间内玩家无心跳超过 5 分钟（如直接关闭 Chrome 窗口），系统自动将其移出房间并释放槽位；更长的宽限期可避免移动端切后台暂停 JavaScript 时被误踢；
...
3. **等待与开赛**：
   * 房间内满 2-4 人时，房主界面会醒目提示并展示“开赛”按钮，点击即可发起对局。开局后所有玩家需先私密整理初始任意牌并确认手牌，全部确认后才进入正常回合。
4. **回合行动逻辑**：
   * **摸牌**：轮到行动的玩家首先从黑色或白色牌堆中摸一张牌，普通牌按数字升序（黑左白右）排列，任意百搭牌（`-` 牌）可选择插入位置。黑白牌堆均耗尽后，回合会直接进入猜牌阶段。
...
5. **对局结算与复盘**：
   * 胜利/完结弹窗中全量公开所有玩家的最终手牌底牌与本局击杀榜，确认无误后点击 `[返回房间待命 (准备下一局)]` 回到等待大厅。
