...
* **主要技术**：
  ...
  * **SSE + Fetch API**：大厅与房间通过 `EventSource` 订阅服务端快照推送；Fetch 用于首次快照和明确的游戏动作。SSE 关闭或失败后客户端会自动重连。
...
3. **🃏 黑白双任意百搭牌 (`-`) 系统与自由插牌摆放**：
   - 牌堆由 24 张扩充至 **26 张**（含 1 张黑色百搭 `-` 与 1 张白色百搭 `-`）。
   - 对局先进入所有玩家同时参与的**私密开局整理**阶段。每位玩家整理并确认初始手牌；阶段结束前，对手只能收到空手牌投影，避免泄露谁拿到了百搭牌。
   - 普通数字牌（0-11）按升序严格自动排列。根据《达芬奇密码》官方经典规则，百搭牌 `-` 允许玩家在**摸牌或手牌回合内自由选择插入到手牌的任意缝隙位置**（最左侧、卡牌之间或最右侧），用于混淆对手心理博弈；手牌区配有 `⇄ 位置` 微调机制。
   - 黑白两个牌堆均耗尽后，下一位存活玩家会直接进入猜牌阶段，不会因无法摸牌而卡住回合。
4. **🛡️ 按观看者投影的对局状态与本局击杀统计**：
   - 未公开的对手牌仅以不透明的按位置标识、颜色和公开状态返回；牌堆的卡牌标识和值同样隐藏。完整状态仅在对局结束后公开。
   - `killStats` 记录本局每位玩家猜中、猜错次数，并驱动房间内与结算页的击杀榜。
5. **🎉 结算弹窗防自动跳转 & 全员手牌公开**：
   - 游戏结束时全量公开所有玩家的最终手牌底牌与百搭牌；
   - 增加确认状态标志，必须由玩家主动点击 `[返回房间待命 (准备下一局)]` 后方可退出结算弹窗并切回等待大厅。
6. **战绩自动结算与排行榜页面 (`/stats`)**：
   - 游戏结束时自动在 `match_history` 插入每位参赛者的胜负及开赛时间，并递增 `users` 表的胜负计数。
   - `/stats` 页面展现个人历史明细与全服胜率排行榜（第 1 名金色 🥇 高亮）。
7. **🟢 全局实时在线人数与 15 秒心跳检测**：
   - 全站底部 Footer 组件每 4 秒发送打卡心跳，在线人数 API 校验 15 秒内有心跳的账号数；关掉标签页 15 秒内自动扣减离线人数。
8. **⚠️ 房间玩家 5 分钟断线/关窗口自动踢出 & 动态日志**：
   - 读取房间时自动比对成员最后心跳时间，无心跳超过 5 分钟后强行踢出房间，释放槽位，并自动转移房主权限；该宽限期适配移动端后台暂停 JavaScript 的情况；
   - 房间等待大厅展示 `📢 房间动态提示`，实时记录加入（`📢`）、离开（`🚪`）与掉线踢出（`⚠️`）。
9. **🔑 房主重返自建加密房间免密码机制**：
   - 房主重返自己创建的加密房间，前端免弹窗，后端比对房主身份无缝放行重返。
10. **回合 30 秒倍数超时提醒机制**：
   - 处于自己回合（`isMyTurn === true`）时触发独立计时器，当无操作停留满 30s、60s、90s... 时，自动弹窗与浮动 Toast 醒目提醒玩家做出选择。
11. **容器化与自动化建表部署 (`Dockerfile` & `entrypoint.sh`)**：
