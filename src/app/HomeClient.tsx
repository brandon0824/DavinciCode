...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (!dialog) return;
      const focusable = Array.from(dialog.querySelectorAll<HTMLElement>('input,button')).filter((element) => !element.hasAttribute('disabled'));
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setSelectedRoomForPassword(null);
    };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    if (authPassword.trim().length < 7) {
      setError('密码长度至少需要7个字符');
      return;
    }
  ...
  const submitPasswordChange = async () => {
    try {
      ...
      setCurrentUser(null); setName('');
      setAuthTab('login');
      setAuthUsername(''); setAuthPassword('');
      setChangePasswordOpen(false); setCurrentPassword(''); setNewPassword('');
  ...
  useEffect(() => {
    ...
    let source: EventSource | undefined; let reconnect: ReturnType<typeof setTimeout> | undefined;
    const connect = () => { if (document.hidden || source) return; source = new EventSource('/api/rooms/events'); source.addEventListener('rooms', event => { const data = JSON.parse((event as MessageEvent).data); setAvailableRooms(data.rooms || []); setIsLoadingRooms(false); }); source.onerror = () => { source?.close(); source = undefined; if (!document.hidden) reconnect = setTimeout(connect, 2000); }; };
    const disconnect = () => { if (source) { source.close(); source = undefined; } if (reconnect) { clearTimeout(reconnect); reconnect = undefined; } };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-200 p-4 sm:p-6 md:p-8 flex items-center">
      <div className="max-w-6xl mx-auto w-full">
        ...
        <AnimatePresence>
          {changePasswordOpen && (
            <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" role="presentation">
              <motion.div id="change-password-dialog" role="dialog" aria-modal="true" aria-labelledby="change-password-title" className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 p-6 space-y-4 shadow-2xl">
                <h3 id="change-password-title" className="text-lg font-bold">请先修改临时密码</h3>
                <Input type="password" placeholder="当前临时密码" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} autoFocus />
                <Input type="password" placeholder="新密码（至少 7 个字符）" value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                <Button className="w-full" onClick={submitPasswordChange} disabled={!currentPassword || newPassword.length < 7}>保存新密码</Button>
              </motion.div>
            </div>
          )}
        </AnimatePresence>
            ...
            <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" role="presentation">
              ...
              >
                <div className="flex items-center space-x-2 text-amber-500">
                  ...
                  <h3 id="room-password-title" className="text-lg font-bold text-slate-800 dark:text-slate-100">
                    该房间受到密码保护
                  </h3>
                </div>
        ...
        <AnimatePresence>
          {error && (
            <motion.div
              ...
              role="alert" aria-live="assertive" className="fixed top-4 right-4 bg-red-500 text-white px-4 py-3 rounded-xl shadow-2xl z-50 max-w-sm flex items-center space-x-2"
            >
