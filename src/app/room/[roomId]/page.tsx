...
export default function RoomPage({ params }: { params: { roomId: string } }) {
  ...
  const drawCard = async (color: 'black' | 'white') => {
    ...
    const cardIndex = deck.findIndex(card => card.color === color);
    
    if (cardIndex === -1) {
      alert(`没有剩余的${color === 'black' ? '黑色' : '白色'}牌了！请选择其他颜色的牌。`);
      return;
    }
    ...
    // Add to player's hand and re-sort
    const newHand = sortCards([...(gameState.hands[playerName] || []), card]);
    ...
    const newLogs = [...gameState.logs, `${playerName} 摸了一张 ${color === 'black' ? '黑色' : '白色'}牌。`];

    const updatedGameState = {
      ...gameState,
      deck,
      hands: updatedHands,
      turnStatus: 'guessing' as const,
      lastDrawnCard: card,
      logs: newLogs
    };
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <Card className="max-w-md bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-2xl rounded-2xl p-4">
          ...
          <CardContent>
            ...
            <Button onClick={() => router.push('/')} className="w-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-white rounded-xl active:scale-[0.96] transition-transform duration-100">
              返回大厅
            </Button>
          </CardContent>
        </Card>
                      ...
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center mb-3">
                          ...
                          {oppEliminated ? (
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-100 text-blue-600 dark:bg-blue-950 dark:text-blue-400 border border-blue-200 dark:border-blue-900/50 rounded-full animate-pulse">
                              思考中
                            </span>
                          ) : null}
                        </div>
