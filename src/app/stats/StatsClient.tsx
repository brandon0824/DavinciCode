      ...
      <div className="max-w-6xl mx-auto w-full space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          <div className="flex items-center space-x-2">
            <Trophy className="w-6 h-6 sm:w-7 sm:h-7 text-amber-500 shrink-0" strokeWidth={2.5} />
        ...
        {/* Content Section */}
        {error && !isLoading && (
          <div role="alert" aria-live="assertive" className="flex items-center justify-between gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-300">
            <span>{error}</span>
            <Button size="sm" variant="outline" onClick={() => fetchStatsData(currentUsername)} className="shrink-0">重试</Button>
          </div>
        )}
        {isLoading ? (
